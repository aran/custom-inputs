...
export default function ApiKeySettings({ onKeyChange }: ApiKeySettingsProps) {
  ...
  return (
    <div className="p-2 bg-zinc-800 rounded-lg">
      <label className="flex items-center gap-1.5 text-xs text-zinc-400 mb-1.5 px-1">
        Anthropic API key
        <PrivacyTooltip />
      </label>
      <div className="flex gap-1.5">
        <input
          ...
          placeholder="sk-ant-..."
          className="flex-1 min-w-0 bg-zinc-900 border border-zinc-700 rounded px-2 py-1.5 text-sm text-zinc-100 placeholder-zinc-500 focus:outline-none focus:border-zinc-500"
        />
        <button
          onClick={handleSave}
          className="px-3 py-1.5 bg-blue-600 hover:bg-blue-500 text-white text-xs rounded shrink-0"
        >
          Save
        </button>
      </div>
